...
const SetupScreen = ({ onGameStart }: SetupScreenProps) => {
  const [step, setStep] = useState<'initial-loading' | 'title' | 'creation' | 'world-loading'>('initial-loading');
  ...
  return (
    // 🌟 로딩 중일 때도 무조건 밝은 하늘색(sky-50) 톤을 유지하도록 수정
    <div 
      onClick={() => step === 'title' && playTitleMusic()}
      className="relative flex h-screen w-full items-center justify-center bg-sky-50 text-slate-800 font-sans overflow-hidden transition-colors duration-1000 ease-in-out"
    >
      <style>{`
        ...
        .anim-fade-in-slow { animation: fadeIn 2s ease-out forwards; }
        .anim-fade-in-up { animation: fadeInUp 1.2s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
        .anim-title { animation: titleFloat 4s ease-in-out infinite; }
      `}</style>

      {/* 🌟 이제 배경 오라와 햇살 레이어는 로딩 화면에서도 항상 나타납니다 */}
      <div className="absolute inset-0 z-0 pointer-events-none anim-fade-in-slow">
      ...
      <div className="absolute inset-0 z-1 overflow-hidden pointer-events-none mix-blend-overlay anim-fade-in-slow">
        <div 
      ...
      {/* 🌟 밝은 배경에 맞춰 수정된 로딩 화면 UI */}
      {(step === 'initial-loading' || step === 'world-loading') && (
        <div className="flex flex-col items-center z-10 anim-fade-in">
          <div className="w-16 h-16 border-4 border-indigo-100 border-t-sky-500 border-r-indigo-600 rounded-full animate-spin mb-8 shadow-[0_0_30px_rgba(99,102,241,0.3)]"></div>
          <p className="text-indigo-600 font-bold tracking-[0.5em] text-sm animate-pulse drop-shadow-sm">
            {step === 'initial-loading' ? 'NOW LOADING...' : 'ENTERING WORLD...'}
          </p>
        </div>
      )}
      ...
      {/* 타이틀 로고 */}
      {step === 'title' && (
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full anim-fade-in-up">
      ...
      {step === 'creation' && (
        <form
          ...
          className="relative z-10 flex w-full max-w-md flex-col gap-5 rounded-3xl bg-white/60 backdrop-blur-2xl p-8 shadow-[0_20px_60px_rgba(0,0,0,0.08)] border border-white/80 anim-fade-in-up"
        >
