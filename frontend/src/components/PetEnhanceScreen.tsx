...
interface PetEnhanceScreenProps {
  player: PlayerStats;
  onClose: () => void;
  onEnhance: () => void;
}

const PetEnhanceScreen = ({ player, onClose, onEnhance }: PetEnhanceScreenProps) => {
  const pet = player.pet;
  const level = pet ? ((player.petEnhanceLevels || {})[pet.id] || 0) : 0;
  const nextBonusPct = (level + 1) * 5;
  const cost = 100 + level * 100;
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="relative w-full max-w-lg rounded-lg bg-white p-6 shadow-xl font-stat" onClick={(e) => e.stopPropagation()}>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-2xl font-bold">🧪 펫 강화소</h2>
            <div className="text-lg">💰 {player.money} G</div>
          </div>
          {pet ? (
            <div>
              <div className="mb-2 text-lg font-semibold">현재 펫: {pet.icon} {pet.name}</div>
              <div className="text-sm text-gray-700 mb-4">현재 보너스: +{level * 5}%</div>
              <div className="rounded border p-4">
                <div className="mb-2 font-medium">다음 강화 효과</div>
                <div className="text-sm">펫 파워 +5% (누적 +{nextBonusPct}%)</div>
            ...
            <div>
              <div className="mb-4">강화할 펫이 없습니다. 상점에서 먼저 펫을 구매하세요.</div>
...
export default PetEnhanceScreen;
