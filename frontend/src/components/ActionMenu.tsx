...
const ActionButton = ({ onClick, disabled, children, hotkey, className = "" }: {
  onClick: () => void;
  disabled: boolean;
            ...
            <>
              ...
              {isBattleSkillOpen && (
                <div className="absolute bottom-full mb-4 flex flex-col gap-1 rounded-lg border border-gray-400 bg-white p-3 shadow-lg z-10 min-w-[200px]">
                  <div className="mb-2 text-center font-bold text-gray-700 border-b pb-1">스킬 목록</div>
                  {learnedSkills.length === 0 ? (
                    <div className="text-center text-gray-500 text-xs py-2">배운 스킬이 없습니다.</div>
                  ) : (
                      ...
                      {learnedSkills.map((key, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {cd > 0 ? (
                              <span className="text-xs text-red-500 font-bold">({cd})</span>
                            ) : (
                              hotkey && <span className="ml-2 rounded bg-gray-200 px-1 text-[10px] font-bold text-gray-600">{hotkey}</span>
                            )}
                          </button>
                        );
                      })}
              ...
              <ActionButton 
                ...
                disabled={!canAct} 
                hotkey="K"
                className={isBattleSkillOpen ? "border-blue-700 bg-blue-100 text-blue-900" : ""}
              >
                스킬
              </ActionButton>
