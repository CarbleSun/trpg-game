...
export const calculateAttack = (
  ...
  logs.push({
    msg: `🗡 ${newAttacker.name}이(가) ${newDefender.name}을(를) 공격한다.`,
    type: "tryToAtk",
  });
  ...
  let damage = Math.ceil(
    newAttacker.atk + atkCalc - (newDefender.def + defCalc)
  );
  ...
  if (newDefender.isDefending) {
    ...
    logs.push({
      msg: `🛡 ${newDefender.name}이(가) 방어했다! (데미지 절반)`,
      type: "normal",
    });
  ...
  if (damage <= 0) {
    ...
    if (!isGuaranteedHit) {
      logs.push({
        msg: `😓 ${newAttacker.name}의 공격이 막혔다! (0 데미지)`,
        type: "fail",
      });
      didHit = false;
  ...
  if (!isCritical && !isGuaranteedHit) {
    ...
    if (getRandom(1, 100) <= evadeRate) {
      logs.push({
        msg: `🍃 ${newDefender.name}이(가) 공격을 회피했다.`,
        type: "fail",
      });
      didHit = false;
      return {
  ...
  if (isGuaranteedHit) {
    ...
    damage += bonusDamage;
    logs.push({
      msg: `✨ 집중력의 일격! ${bonusDamage}의 추가 데미지!`,
      type: "vic",
    });
    didHit = true;
  }
